...
## Simulation for the heterogenous setting with same cores, run on NYU High Performance Computing
...
# simulation: run the simulation experiments for the heterogeneous tensors with the same core. Three 
...
def simulation(J, p, r_U, r_V, L, sigma, gap_seq, reps=100, seed=0):
    ...
    for gap in gap_seq:
        ...
        for rep in range(reps):
            ...
            T_star_list = [tl.tucker_tensor.tucker_to_tensor((G, UV_list[l])) for l in range(L)]
...
# Save the results
np.savetxt(f"results/hetero_res_p={p}_L={L}_gamma={gamma}.txt", res_mat)